...
//                        writln(#9'EXAB'#9#9'; Store only HB in byte var!');
                ...
                begin
                    ...
                    begin // Local char
                        writln(#9'EXAB');
                        writln(#9'LDR');
                        writln(#9'ADIM'#9+inttostr(adr+2+pushcnt));
                        writln(#9'STP');
                        writln(#9'EXAB');
                        writln(#9'EXAM');
                    end;
            ...
            begin
                ...
                begin
                    ...
                    begin // Local word
                        ...
                        writln(#9'LDR');
                        writln(#9'ADIM'#9+inttostr(adr+1+pushcnt));
                        writln(#9'STP');
        ...
        begin
                ...
                begin
                        ...
                        begin
                                ...
                                writln( #9'EXAM');
                                writln( #9'LDR');
                                writln( #9'ADIM'#9+inttostr(a)); dec(pushcnt, a);
                                writln( #9'STR');
                                writln( #9'EXAM');
                        end else
...
begin
        ...
        m := VarList[VarFound].address;
        for i := 0 to lc - 1 do
        begin
                name := ProcList[currproc].locname[i];
                if not Findvar(name) then error('Var '+name+' not declared!');
                if not VarList[VarFound].local then error('Var '+name+' not local!');
                VarList[VarFound].address := m - VarList[VarFound].address;
        end;
end;
...
//        writln('');
        ...
        begin
                readln(f, s);
                if copy(trim(s),1,5) = 'LIB'#9'1' then
                begin
                        name := s;
                        readln(f, s);
