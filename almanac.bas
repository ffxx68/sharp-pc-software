...
50 REM WORKING IN DEGREE MODE (NOT RADIAN)
60 REM RD=PI/180:REM DEGREES TO RADIANS
65 REM DR=180/PI:REM RADIANS TO DEGREES
...
580 GH=GM-RA+180
585 IF GH>=360 THEN LET GH=GH-360
590 IF GH<0 THEN LET GH=GH+360
600 REM CALCULATE DECLINATION (ARCSIN FORMULA UNA SOLA VOLTA)
...
685 IF SS>1 THEN LET SS=1
686 IF SS<-1 THEN LET SS=-1
...
735 IF ABS(CZ)<.01 THEN LET CZ=SGN(CZ)*.01
740 ZN=ATN(SQR(1-CZ*CZ)/CZ)
750 IF CZ<0 THEN LET ZN=ZN+180
760 IF SIN(LH)<0 THEN LET ZN=360-ZN
770 REM INTERCEPT
780 I=HO-HC
860 REM GHA
870 GD=INT(GH)
...
960 REM DECLINATION
970 DD=INT(ABS(DC))
980 DM=INT((ABS(DC)-DD)*60)
990 DS=INT(((ABS(DC)-DD)*60-DM)*60)
